...
function UILibrary.new(gameName, userId, rank)
    local OldUI = getgenv().UIAlreadyUsed;
    if OldUI then
        OldUI:Destroy()
    end;
    ...
    --// Customize the GUI
    window.Watermark.Text = ("Nord | %s | %s"):format(userId, gameName)
...
function UILibrary.Window:Close()
    getgenv().UIAlreadyUsed:Destroy()
    getgenv().UIAlreadyUsed = nil;
end
